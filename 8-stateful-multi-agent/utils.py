...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        ...
        current_state["interaction_history"].append(entry_with_timestamp)

        # Create an event with the updated state using the correct structure
        from google.genai import types
        
        # Create a content part for the event
        content = types.Content(
            role="system",
            parts=[types.Part(text="Interaction history updated")]
        )
        
        # Create state delta with the updated state
        state_delta = {"interaction_history": current_state["interaction_history"]}
        
        # Create the event with the state delta
        event = types.Event(
            author="system",
            content=content,
