...
class GeoAgent:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.bq_client = bigquery.Client() if self.config.get(
            'use_bigquery', False) else None
        if self.bq_client:
        ...
        try:
            ...
            mock_geocoding = {
                ...
                'geometry': {
                    'location': {
                        # Near Sydney
                        'lat': -33.8688 + np.random.uniform(-0.1, 0.1),
    ...
    def aggregate_by_geography(self,
        ...
        try:
            ...
            df_geo = df.copy()
            df_geo['lat_bin'] = (
                df_geo[lat_col] / resolution).round() * resolution
            df_geo['lon_bin'] = (
                df_geo[lon_col] / resolution).round() * resolution
            ...
            df_agg = df_geo.groupby(['lat_bin', 'lon_bin']).agg(
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    logger.warning(
                        f"Skipping row with invalid coordinates: {e}")
                    continue
    ...
    def query_bigquery_geo(self,
        ...
        if not self.bq_client:
            raise RuntimeError(
                "BigQuery client not initialized. Set use_bigquery=True in config.")
        ...
        try:
            if operation == 'geocode':
                ...
                if not isinstance(data, str):
                    raise ValueError(
                        "For geocoding, data must be an address string")

                api_key = params.get('api_key') or self.config.get(
                    'geocoding_api_key')
                ...
                if not isinstance(params.get('point1'), dict) or not isinstance(params.get('point2'), dict):
                    raise ValueError(
                        "For distance calculation, params must include 'point1' and 'point2' with lat/lng")
                ...
                if isinstance(data, str):
                    if data.strip().lower().startswith('select'):
                        # It's a SQL query
                        df = self.query_bigquery_geo(
                            data, params.get('query_params'))
                    else:
                        ...
                        if data.endswith('.csv'):
                            ...
                            raise ValueError(
                                f"Unsupported data source: {data}")
                elif isinstance(data, pd.DataFrame):
                    df = data.copy()
                else:
                    raise ValueError(
                        "Input data must be a DataFrame, file path, or SQL query")
                ...
                if properties is None:
                    ...
                    lon_col = params.get('lon_col', 'longitude')
                    properties = [
                        col for col in df.columns if col not in [lat_col, lon_col]]
