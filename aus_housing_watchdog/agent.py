...
class CoordinatorAgent:
    ...
    def run_pipeline(self, pipeline_name: str = 'daily', params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        pipeline_steps = self.config.get(
    ...
    def _execute_step(self, step_name: str, params: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(agent, action):
                ...
                raise ValueError(
                    f"Agent '{agent_name}' has no method '{action}'")
    ...
    def _get_default_pipeline(self, pipeline_name: str) -> List[Dict]:
        ...
        if pipeline_name == 'daily':
            return [
                {'name': 'ingestion.fetch', 'params': {
                    'source': 'all'}, 'on_error': 'stop'},
                {'name': 'cleaning.process', 'params': {
                    'source': 'ingestion'}, 'on_error': 'stop'},
                {'name': 'trend.analyze', 'params': {
                    'source': 'cleaning'}, 'on_error': 'continue'},
                {'name': 'anomaly.detect', 'params': {
                    'source': 'cleaning'}, 'on_error': 'continue'},
                {'name': 'geo.aggregate', 'params': {
                    'source': 'cleaning'}, 'on_error': 'continue'},
                {'name': 'report.generate', 'params': {
                    'report_type': 'daily_summary'}, 'on_error': 'continue'}
            ]
        elif pipeline_name == 'weekly':
            return [
                {'name': 'ingestion.fetch', 'params': {
                    'source': 'all', 'full_refresh': True}, 'on_error': 'stop'},
                {'name': 'cleaning.process', 'params': {
                    'source': 'ingestion'}, 'on_error': 'stop'},
                {'name': 'trend.analyze', 'params': {'source': 'cleaning',
                                                     'time_window': '7d'}, 'on_error': 'continue'},
                {'name': 'anomaly.detect', 'params': {
                    'source': 'cleaning'}, 'on_error': 'continue'},
                {'name': 'geo.aggregate', 'params': {'source': 'cleaning',
                                                     'resolution': 0.05}, 'on_error': 'continue'},
                {'name': 'report.generate', 'params': {
                    'report_type': 'weekly_analysis'}, 'on_error': 'continue'}
            ]
            ...
            return [
                ...
                {'name': 'cleaning.process', 'params': {
                    'source': 'ingestion'}, 'on_error': 'stop'},
                {'name': 'trend.analyze', 'params': {'source': 'cleaning',
                                                     'time_window': '30d'}, 'on_error': 'continue'},
                {'name': 'anomaly.detect', 'params': {'source': 'cleaning',
                                                      'sensitivity': 'high'}, 'on_error': 'continue'},
                {'name': 'geo.aggregate', 'params': {'source': 'cleaning',
                                                     'resolution': 0.02}, 'on_error': 'continue'},
                {'name': 'report.generate', 'params': {
                    'report_type': 'monthly_report'}, 'on_error': 'continue'}
            ]
        else:
    ...
    def run_analysis(self, analysis_type: str = 'all', **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'cleaning' in self.state['results']:
                ...
                raise ValueError(
                    "No cleaned data available and no source provided")

            results = {}

            # Run the requested analysis
            if analysis_type in ['trend', 'all']:
                trend_result = self.agents['trend'].run(
                    data, **kwargs.get('trend_params', {}))
                results['trend'] = trend_result

            if analysis_type in ['anomaly', 'all']:
                anomaly_result = self.agents['anomaly'].run(
                    data, **kwargs.get('anomaly_params', {}))
                results['anomaly'] = anomaly_result

            if analysis_type in ['geo', 'all']:
                geo_result = self.agents['geo'].run(
                    data, **kwargs.get('geo_params', {}))
